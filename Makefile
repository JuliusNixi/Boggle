...
# This below post flag (-lm) is needed because the code (server) use the <math.h> lib.
...
SRCC = $(NC).c
SRCC2 = $(SC).c
# Source common file NAMES.
SRCU = $(SU).c

# Header server file NAMES.
HS = $(SS).h
# Header client file NAMES.
HC = $(SC).h
# Header common file NAMES.
HU = $(SU).h

# Special current game matrix valid words.
...
.PHONY: all clear clean execs execc

# The first target is the default one, called with "make".
# Targets bins of client and server.
all: clear $(EXES) $(EXEC)
...
# Delete all .o and exe files.
	-rm -f $(EXES) $(EXEC) $(BINDIR)$(OBJS) $(BINDIR)$(OBJC) $(BINDIR)$(OBJSS) $(BINDIR)$(OBJSC) $(BINDIR)$(OBJSU) $(ROOTDIR)$(RNS) $(ROOTDIR)$(RNC)
# Delete all testing logs file.	
	-rm -f $(TESTSDIR)/Python/Logs/*.txt $(TESTSDIR)$(SP)
