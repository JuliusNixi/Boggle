...
This file is a test to see if it's possible to write data structures represented by multiples
bytes, using multiples different write() calls, writing one byte each time,
by properly updating the generic void* pointer used. 

After testing it's confirmed, I can use multiple writes to write data simply by incrementing 
...
int main(void) {
    ...
    // I expect to see 97 obviously if now i print x.
    printf("%u.\n", (x));
    ...
    // The first byte is all 0. The second byte is 0000 0001 ...other 2 bytes here.
