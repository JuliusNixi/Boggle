...
void* disconnecterCheckerT(void* args) {

    int retvalue;
    fprintf(stdout, "I'm the disconnecterCheckerT() thread!\n");
...
int main(void) {
    ...
    if (p == 0) {
        ...
        // Important to not close the socket, to see if this solution solves the problem.
        ...
        pthread_t t;
        retvalue = pthread_create(&t, NULL, disconnecterCheckerT, NULL);
        if (retvalue != 0) {
            // Error
        }
        fprintf(stdout, "Thread disconnecterCheckerT() created!\n");
